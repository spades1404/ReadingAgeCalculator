...
def mainbox():
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def createFF():#Func to create factfile
        if calculate() == 1:#Checks if user inputted anything
            return()
        messagebox.showinfo(title = "Notice",message = "The email will not contain the intended grade or grade difference")#Notice message
        FF = createmessage()#FF is the factfile text
        ffBox= Tk()#Creates box
        ffBox.attributes("-topmost", True)#Keeps it above everything
        def saveFF():#Func to actually save factfile
            filename = ffEnt.get()#Gets input from user
            if filename.replace(" ","") == "":#Error checking for blank filenames
                messagebox.showwarning(title = "Error 1", message = "Enter a filename")#Error message
                return()
            ffBox.destroy()#Deletes box
            ffDir = desktop + r'/' + filename + '.txt'#Craeting filename
            with open(ffDir,'w') as file:#Creating File
                file.write(FF)
            messagebox.showinfo(title="Saved", message = "File saved on desktop")#Notice message
        #Constructing a window
        Label(ffBox,text = "Type in the filename below").pack()
        ffEnt = Entry(ffBox)
        ffEnt.pack()#geometry manager
        Button(ffBox, text = "Create",command=saveFF).pack()#Button for user

        ffBox.mainloop()#Keeps box running until stopped
...
type in your email. Click 'Send' and its done!'''
            ...
            if tutSC == 11:
                backButton['state'] = 'normal'
                picLabel['image'] = img13
                textLabel['text'] = '''If you want to share your info click 'Create Factfile' to make a text file with all your info'''
            if tutSC == 12:
                backButton['state'] = 'normal'
...
If entering a large text (300-400 pages) you may need a higher performance PC'''
            if tutSC == 15:#Means that tutorial is finished
...
##
    ...
    mainmenu.add_command(label="Create Factfile",command = createFF)
